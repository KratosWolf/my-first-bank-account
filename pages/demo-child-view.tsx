...
export default function ChildView() {
  ...
  const [newGoalData, setNewGoalData] = useState({ name: '', amount: '', category: '' });
  const [newLoanData, setNewLoanData] = useState({ reason: '', amount: '', category: '' });
  ...
  const payLoan = async () => {
    ...
    alert(`✅ Pagamento realizado! ${amount >= activeLoan.amount ? 'Empréstimo quitado!' : `Restam R$ ${(activeLoan.amount - amount).toFixed(2)}`}\nNovo saldo: R$ ${newBalance.toFixed(2)}`);
  };

  const contributeToGoal = async (goalName: string) => {
    const currentBalance = currentChild?.balance || 0;
    const amountStr = prompt(`Quanto você quer guardar para "${goalName}"?\n\nSeu saldo: R$ ${currentBalance.toFixed(2)}`);
    ...
    alert(`✅ R$ ${amount.toFixed(2)} guardado para ${goalName}!\nNovo saldo: R$ ${newBalance.toFixed(2)}`);
  };
  ...
  const requestPurchase = async (category: string) => {
    ...
    const currentBalance = currentChild.balance || 0;
    const itemName = prompt(`O que você quer comprar na categoria "${category}"?`);
    if (!itemName) return;
    
    const amountStr = prompt(`Quanto custa "${itemName}"?\n\nSeu saldo: R$ ${currentBalance.toFixed(2)}`);
    if (!amountStr) return;
    ...
    try {
      ...
      alert(`📨 Pedido enviado para seus pais!\n\nItem: ${itemName}\nCategoria: ${category}\nValor: R$ ${amount.toFixed(2)}\n\nVocê pode acompanhar na tela inicial! 😊`);
    } catch (error) {
      console.error('❌ Erro interno:', error);
      alert('❌ Erro interno ao enviar pedido. Verifique sua conexão e tente novamente.');
    }
  };
  ...
  const submitNewGoal = async () => {
    ...
    if (!newGoalData.name || !newGoalData.category || isNaN(amount) || amount <= 0) {
    ...
    const selectedCategory = availableCategories.find(cat => cat.name === newGoalData.category);
    
    try {
      // Criar solicitação de meta no Supabase
      const purchaseRequest = await DatabaseService.createPurchaseRequest(currentChild.id, {
        type: 'goal',
  ...
  const submitLoanRequest = () => {
    ...
    if (!newLoanData.reason || !newLoanData.category || isNaN(amount) || amount <= 0) {
      alert('❌ Preencha todos os campos corretamente!');
      return;
    }

    const selectedCategory = availableCategories.find(cat => cat.name === newLoanData.category);
  ...
  const childData = {
    ...
    goals: realGoals.length > 0 ? realGoals.map(goal => ({
      id: goal.id,
      name: goal.name,
    ...
    recentTransactions: realTransactions.length > 0 ? realTransactions.slice(0, 5).map(tx => ({
      id: tx.id,
      type: tx.type as any,
      amount: tx.amount,
      description: tx.description || tx.category,
      date: new Date(tx.created_at).toLocaleDateString('pt-BR'),
      icon: tx.type === 'allowance' ? '💰' : 
            tx.type === 'purchase' ? '🛒' : 
            tx.type === 'goal_contribution' ? '🎯' : '💸'
    })) : recentTransactions
  };

  const completionPercentage = Math.round((childData.xp / (childData.xp + childData.xpToNext)) * 100);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-xl font-bold text-gray-700 mb-2">Carregando seu banco...</div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50">
        ...
        <div className="max-w-md mx-auto px-4 py-6">
          <div className="flex items-center space-x-4">
            ...
            <div className="text-right flex flex-col items-end space-y-2">
              ...
              <div>
                <div className="text-2xl font-bold">R$ {childData.balance.toFixed(2)}</div>
          ...
          <div className="mt-6 bg-white/20 rounded-xl p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm">{childData.xp} / {childData.xp + childData.xpToNext} XP</span>
            </div>
      ...
      <div className="max-w-md mx-auto">
        ...
        {selectedTab === 'home' && (
          <div className="px-4 py-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-green-500 to-emerald-500 rounded-3xl p-8 text-white text-center shadow-xl">
              <div className="text-5xl font-bold mb-2">R$ {childData.balance.toFixed(2)}</div>
              <div className="text-green-100 text-lg">Meu Dinheiro Disponível</div>
              <div className="mt-6">
                <button 
            ...
            {childData.activeLoan && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-yellow-800">Empréstimo Ativo</div>
                    <div className="text-sm text-yellow-600">
                      R$ {childData.activeLoan.amount.toFixed(2)} - {childData.activeLoan.reason}
                    </div>
                    <div className="text-xs text-yellow-500">Empréstimo sem juros</div>
                  </div>
                  <button 
                    onClick={() => setSelectedTab('use-money')}
            ...
            <div className="bg-gradient-to-r from-orange-400 to-red-500 rounded-2xl p-6 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-3xl font-bold">{childData.currentStreak} dias</div>
                  <div className="text-orange-100">Sequência de tarefas! 🔥</div>
                </div>
            ...
            {childData.balance >= 10 && (
              <div className="bg-gradient-to-r from-green-400 to-blue-500 rounded-2xl p-6 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl font-bold">R$ {(childData.balance * 0.01).toFixed(2)}</div>
                    <div className="text-green-100">Seu dinheiro está rendendo! 📈</div>
                  </div>
                  <div className="text-4xl">💰</div>
                </div>
                <div className="mt-3 text-sm opacity-90">
                  Com R$ {childData.balance.toFixed(2)} na conta, você pode ganhar até R$ {(childData.balance * 0.01).toFixed(2)} por mês
                </div>
              ...
              <div className="bg-gradient-to-r from-gray-400 to-gray-500 rounded-2xl p-6 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-gray-100">Meta para começar a render! 💰</div>
                  </div>
                  <div className="text-4xl">🎯</div>
                </div>
                <div className="mt-3 text-sm opacity-90">
                  Economize mais R$ {(10 - childData.balance).toFixed(2)} para seu dinheiro começar a render 1% ao mês!
                </div>
                <div className="mt-1 text-xs opacity-75">
                  ⏰ Condição: O dinheiro precisa ficar 30 dias na conta para render
                </div>
                <div className="mt-1 text-xs opacity-60">
                  📊 Taxa: 1% ao mês • Mínimo: R$ 10,00
                </div>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-600">
                            {request.type === 'purchase' ? `Compra: ${request.category}` :
                             request.type === 'goal' ? `Sonho: ${request.category}` :
                             `Empréstimo: ${request.category}`} • R$ {request.amount.toFixed(2)}
                          </div>
                          <div className="text-xs text-orange-600">
                            Enviado {new Date(request.created_at).toLocaleDateString('pt-BR')}
                          </div>
                        </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="font-semibold text-blue-800">Próxima Mesada</div>
                  <div className="text-sm text-blue-600">R$ {childData.weeklyAllowance.toFixed(2)} na {childData.nextAllowanceDate}</div>
                </div>
              </div>
            </div>
                ...
                {childData.recentTransactions.slice(0, 3).map(transaction => (
                  <div key={transaction.id} className="p-4 flex items-center space-x-3">
                    ...
                    <div className={`font-bold ${
                      ...
                      transaction.type === 'loan_payment' ? 'text-orange-600' :
                      'text-red-500'
                    }`}>
                      {transaction.type === 'received' ? '+' : 
                       transaction.type === 'loan' ? '+' :
                       transaction.type === 'loan_payment' ? '-' : '-'}R$ {transaction.amount.toFixed(2)}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {selectedTab === 'use-money' && (
          <div className="px-4 py-6 space-y-6">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Usar Meu Dinheiro</h2>
              <div className="text-lg text-green-600 font-semibold">R$ {childData.balance.toFixed(2)} disponível</div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border-2 border-blue-300 rounded-xl p-6">
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <div>
                    <h3 className="font-bold text-blue-800">Pedir Emprestado</h3>
                    <p className="text-sm text-blue-600">Empréstimo sem juros dos seus pais</p>
                  </div>
                </div>
                <button 
                  onClick={requestLoan}
              ...
              {childData.activeLoan && (
                <div className="bg-yellow-50 border-2 border-yellow-300 rounded-xl p-6">
                  <div className="flex items-center space-x-3 mb-4">
                    <div className="text-3xl">🏦</div>
                    <div>
                      <h3 className="font-bold text-yellow-800">Pagar Empréstimo</h3>
                      <p className="text-sm text-yellow-600">R$ {childData.activeLoan.amount.toFixed(2)} - {childData.activeLoan.reason}</p>
                    </div>
                  </div>
                  <button 
                    onClick={payLoan}
              ...
              <div className="bg-purple-50 border-2 border-purple-300 rounded-xl p-6">
                <div className="flex items-center space-x-3 mb-4">
                  <div className="text-3xl">🎯</div>
                  <div>
                    <h3 className="font-bold text-purple-800">Guardar para Sonho</h3>
                    <p className="text-sm text-purple-600">Contribuir para uma das suas metas</p>
                  </div>
                </div>
                <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="flex-1 text-left">
                        ...
                        <div className="text-xs text-gray-500">
                          R$ {goal.current.toFixed(2)} de R$ {goal.target.toFixed(2)} ({Math.round((goal.current / goal.target) * 100)}%)
                        </div>
              ...
              <div className="bg-blue-50 border-2 border-blue-300 rounded-xl p-6">
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-600">Escolher algo para comprar</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <div className="text-sm font-semibold text-gray-900">Jogos</div>
                  ...
                  >
                    <div className="text-2xl mb-1">👕</div>
                    <div className="text-sm font-semibold text-gray-900">Roupas</div>
                    <div className="text-xs text-gray-500">Pedir aprovação</div>
                  </button>
                  <button 
                  ...
                  >
                    <div className="text-2xl mb-1">📚</div>
                    <div className="text-sm font-semibold text-gray-900">Livros</div>
                    <div className="text-xs text-gray-500">Pedir aprovação</div>
                  </button>
                  <button 
                  ...
                  >
                    <div className="text-2xl mb-1">⚽</div>
                    <div className="text-sm font-semibold text-gray-900">Esportes</div>
                    <div className="text-xs text-gray-500">Pedir aprovação</div>
                  </button>
                </div>
              </div>
        ...
        {selectedTab === 'goals' && (
          <div className="px-4 py-6 space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-bold text-gray-900">Meus Sonhos 🌟</h2>
              <button
            ...
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-200 rounded-xl p-6 text-center">
              ...
              <h3 className="font-semibold text-purple-800 mb-2">Tem um novo sonho?</h3>
        ...
        {selectedTab === 'achievements' && (
          <div className="px-4 py-6 space-y-4">
            <h2 className="text-xl font-bold text-gray-900">Minhas Conquistas 🏆</h2>
            <div className="grid grid-cols-2 gap-4">
              {childData.achievements.map(achievement => (
                <div key={achievement.id} className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-xl p-4 text-white text-center">
                  <div className="text-3xl mb-2">{achievement.icon}</div>
                  <div className="font-bold text-sm">{achievement.name}</div>
                  <div className="text-xs text-yellow-100">{achievement.earnedAt}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {selectedTab === 'history' && (
          <div className="px-4 py-6 space-y-4">
            <h2 className="text-xl font-bold text-gray-900">Meu Histórico 📋</h2>
            <div className="space-y-3">
              {childData.recentTransactions.map(transaction => (
                <div key={transaction.id} className="bg-white rounded-xl p-4 shadow-lg flex items-center space-x-3">
                  <div className="text-2xl">{transaction.icon}</div>
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">{transaction.description}</div>
                    <div className="text-xs text-gray-500">{transaction.date}</div>
                  </div>
                  <div className={`font-bold ${transaction.type === 'received' ? 'text-green-600' : 'text-red-500'}`}>
                    {transaction.type === 'received' ? '+' : '-'}R$ {transaction.amount.toFixed(2)}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 px-4 py-2">
          <div className="flex justify-around">
            <TabButton id="home" icon="🏠" label="Início" isActive={selectedTab === 'home'} onClick={setSelectedTab} />
            <TabButton id="goals" icon="🎯" label="Sonhos" isActive={selectedTab === 'goals'} onClick={setSelectedTab} />
            <TabButton id="achievements" icon="🏆" label="Conquistas" isActive={selectedTab === 'achievements'} onClick={setSelectedTab} />
            <TabButton id="history" icon="📋" label="Histórico" isActive={selectedTab === 'history'} onClick={setSelectedTab} />
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-gray-900">🌟 Criar Novo Sonho</h2>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewGoalData({...newGoalData, amount: e.target.value})}
      ...
      {showLoanModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-gray-900">🏦 Pedir Empréstimo</h2>
              <button
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              <p className="text-sm text-blue-700">
                💡 Lembre-se: Empréstimos são <strong>sem juros</strong>, mas precisam ser pagos de volta!
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewLoanData({...newLoanData, category: e.target.value})}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  <option value="">Escolha uma categoria...</option>
                  {availableCategories.map(cat => (

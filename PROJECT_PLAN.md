...
- 🔄 Fase 3 em progresso — 13/19 tasks completas
...
**Status:** 🔄 Em progresso — 13/19 tasks completas (3.1, 3.2, 3.3a, 3.5, 3.6, 3.7, 3.9, 3.10, 3.11, 3.12, 3.13, 3.14, 3.15).
...
| 3.18 | Transações antigas com carimbo `pending` errado + fluxos de sonhos | 🔒     | **17 transações pendentes** no banco, das quais 15 são de tipo-entrada: **13 `earning` (R$1.850,00)** e **2 `interest` (R$63,66)** de jan e fev/2026 — R$1.913,66 no total. Mais **1 `spending`** (R$285,02, 22/12/2025) e **1 `goal_withdrawal`** (R$285,02, 23/02/2026). **O saldo JÁ está correcto** (confirmado pelo Tiago e por SELECT) — portanto **NÃO creditar**. É só questão de histórico: não aparecem no extracto. **O bloqueio:** corrigir o carimbo torna-as visíveis, mas o saldo acumulado do extracto é calculado **de trás para a frente** a partir do saldo actual — fazer aparecer R$1.913 de entradas antigas quebra todos os saldos históricos mostrados à criança. **Resolver isso ANTES de mexer nos carimbos.** Ficaram também com `requires_approval=false`, logo invisíveis na lista de pendentes do painel: limbo — marcadas como não-confirmadas e sem ninguém que as pudesse confirmar. **Avaliar aqui também:** (i) `goal-contributions.js` grava `type='spending'` com um comentário a dizer que `'goal_deposit'` não existe no schema, mas `add-money.js` grava `'goal_deposit'` — dois ficheiros a fazer depósitos em sonhos com tipos diferentes, possível duplicação ou código morto; (ii) `goal_purchase` do `resolve-fulfillment.js` **não move** `children.balance` (L121 mantém o saldo de propósito) mas o extracto trata-o como despesa no walk-back — ver nota no CLAUDE.md.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

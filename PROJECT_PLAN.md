...
- 🔄 Fase 3 em progresso — 9/17 tasks completas
...
**Status:** 🔄 Em progresso — 9/17 tasks completas (3.1, 3.2, 3.3a, 3.5, 3.6, 3.7, 3.9, 3.10, 3.12).
...
| 3.11 | Migrar crons para agendador externo + alerta    | 🔴     | **PRIORITÁRIA — é a causa raiz de tudo.** O GitHub desativa cada workflow agendado **INDIVIDUALMENTE**, no instante do seu próprio disparo seguinte após 60 dias de inatividade do repo — não todos em bloco. Por isso `daily-allowance` e `keep-alive` caíram a 05/08 (diários) e `monthly-interest` só a 01/09 (mensal). A última execução antes do silêncio é **SEMPRE** `success`, portanto nenhuma validação de corpo de resposta deteta isto. **Só um watchdog de NÃO-EXECUÇÃO deteta.** O watchdog tem de tolerar atraso de várias horas: o cron está agendado para 08:00 UTC e correu às 12:48 UTC (4h48 de atraso); o `monthly-interest` está agendado para 00:00 UTC e corre entre 02:00 e 04:30. Migrar para agendador externo (Vercel Cron, Supabase pg_cron ou serviço dedicado) com alerta ativo. Incluir também a validação do corpo da resposta no `daily-allowance.yml`, que hoje usa apenas `-s -S` — um HTTP 500 produz job verde. |
...
| 3.13 | Release Pipeline falhando desde sempre          | 🔴     | semantic-release sem `.releaserc` — o job falha a cada push e gera email de alarme falso, que treina o operador a ignorar alertas. Configurar ou remover o workflow. **Ligada à 3.11.** O Release Pipeline falha a cada push (incluindo commits só de documentação) e gera email de alarme falso. Foi isso que anulou os avisos **REAIS** do GitHub — “will be disabled soon” recebidos a 05/06 e 29/07/2026, ambos ignorados. O alarme falso é parte da causa raiz, não um incómodo cosmético.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 3.15 | Endurecer apply-allowance (3 defeitos latentes) | 🔒     | Descobertos no diagnóstico da 3.12 — nenhum causou a falha de setembro, todos mordem depois. **(a)** A guarda de idempotência (`hasAllowanceInMonth`, ~L269-273) devolve `true` em caso de **ERRO** na query, o que faz o código registar `skipped_idempotent`, avançar `next_payment_date` e devolver HTTP 200. Um erro transitório queima um mês de mesada em silêncio. Deve abortar aquela criança **SEM** avançar a data e devolver HTTP 500. **(b)** `calculateNextPaymentDateFrom` tem `default: return fromDate` (~L325). Uma `frequency` desconhecida faz o `while` da L92 nunca terminar. Trocar por erro explícito. **(c)** Regra 6 — o código trata `'biweekly'` mas o CHECK de `allowance_config` só permite `daily                                                                                                                                                                                                                       | weekly | monthly`. Decidir: remover do código ou adicionar ao CHECK. |

...
**Status:** 🔄 Em progresso — 8/13 tasks completas (3.1, 3.2, 3.3a, 3.5, 3.6, 3.7, 3.9, 3.10).
**Saldos actuais (2026-09-08):** Gabriel R$415,05 / Rafael R$129,73.
...
| 3.11 | Migrar crons para agendador externo + alerta  | 🔴     | **PRIORITÁRIA — é a causa raiz de tudo.** Os 3 workflows agendados do GitHub Actions são desativados automaticamente após 60 dias sem atividade no repo, e isso derrubou mesada e juros silenciosamente. Migrar para agendador externo (Vercel Cron, Supabase pg_cron ou serviço dedicado) com alerta ativo de falha — o operador tem de ser notificado quando o cron não roda, não só quando roda e falha.                                                                                                                                                                                                                                                                                                                                                                                                |
| 3.12 | Mesada de setembro não creditada              | 🔴     | O cron de 08/09 rodou e não pagou; `next_payment_date` continua 2026-09-05. **Investigar o filtro de data no apply-allowance antes de creditar** (suspeita de `.eq` em vez de `.lte` nalgum caminho). Só depois creditar os R$100 de cada criança. Não creditar antes de entender a causa.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 3.13 | Release Pipeline falhando desde sempre        | 🔒     | semantic-release sem `.releaserc` — o job falha a cada push e gera email de alarme falso, que treina o operador a ignorar alertas. Configurar ou remover o workflow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |

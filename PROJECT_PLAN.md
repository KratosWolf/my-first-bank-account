...
- 🔄 Fase 3 em progresso — 13/21 tasks completas
...
**Status:** 🔄 Em progresso — 13/21 tasks completas (3.1, 3.2, 3.3a, 3.5, 3.6, 3.7, 3.9, 3.10, 3.11, 3.12, 3.13, 3.14, 3.15).
...
| 3.6  | Fix exibição de goal_withdrawal no histórico                         | ✅     | goal_withdrawal exibe +R$ em verde (entrada); goal_deposit continua -R$ em vermelho; saldo no banco não alterado (já correto); build OK; commit feito. ⚠️ **Nota (08/09, Task 3.14):** o efeito da correcção só alcança transações **novas**. A única `goal_withdrawal` que existe no banco (R$285,02, 23/02/2026) nasceu com `status='pending'` por causa do DEFAULT da coluna, e o extracto filtra `status='completed'` desde 05/04/2026 — continua invisível. A migration 007 não a corrige retroactivamente; essa linha vai para a 3.18.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 3.7  | Simulação de saldo após aprovação no card                            | ✅     | Dashboard pais mostra "Saldo após aprovação: -R$X" quando saldo insuficiente. Não aparece quando saldo é suficiente. Build OK. **Observação (08/09, não é bug):** a API de aprovação não bloqueia aprovações que deixem o saldo baixo ou negativo — o painel avisa, a API confia no aviso.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 3.14 | Fechar a porta que cria transações `pending` por acidente            | ✅     | **Causa raiz real:** o DEFAULT de `transactions.status` era `'pending'` — qualquer INSERT que omitisse o campo criava uma transação não-confirmada em silêncio. Migration `007_transactions_status_default_completed.sql`: `'pending'` → `'completed'`. **Só o DEFAULT**; zero linhas de dados alteradas (contagens por status idênticas antes/depois, 9 grupos, 75 tx). **CORRECÇÃO AO DIAGNÓSTICO ANTERIOR:** a causa **não** era "a regra dos 30 dias não filtra por `status`". As tx de R$50 do Gabriel de 17/05 são `type='spending'`/`status='rejected'` e `spending` nem está em `INCOME_TYPES_30D`; nenhuma pendente cai na janela de Outubro. Impacto medido dessa query: **R$0,00**. O filtro `.eq('status','completed')` **não** foi aplicado ao `apply-interest` de propósito — seria penso em cima do sintoma. **Descoberto durante a Fase 1** (fora do inventário inicial): quatro INSERTs vivos que omitiam `status` — `goal-contributions.js:127`, `goals/resolve-fulfillment.js:100`, `goals/cancel.js:140`, `goals/add-money.js:165`. Todos passaram a declarar `status: 'completed'` (diff: 4 ficheiros, 4 inserções, zero remoções). **Código:** `addEarning`/`addAllowance`/`requestSpending` apagadas de `transactions.ts` (zero chamadores, confirmado por grep; `requestSpending` tinha um segundo defeito — recebia `requiresApproval` e nunca o usava). `database.ts:createTransaction` **endurecida, não apagada** (chamadores reais em `storage-adapter.ts:88` e `:280`): a actualização do saldo passa a exigir `status === 'completed'` — era esta função que explicava o desalinhamento "saldo certo, ficha por assinar". **Intocados por design:** `purchase-requests.js` (escreve `'pending'` explícito em 3 sítios, incluindo a reversão quando o débito falha) e `apply-interest.ts`. As 17 transações pendentes antigas não foram tocadas → Task 3.18. |
...
| 3.18 | Fluxos de sonhos: erro engolido, tipos duplicados e retenção de logs | 🔒     | **As 17 transações pendentes antigas SAÍRAM do âmbito desta task.** Decisão do Tiago (08/09): **não se tocam**. A reconciliação por SELECT confirma que o dinheiro **está** no saldo — a perda é de histórico, não de dinheiro — e torná-las visíveis quebraria os saldos históricos do extracto, calculados de trás para a frente a partir do saldo actual. Ficaram também com `requires_approval=false`, logo invisíveis na lista de pendentes do painel: limbo, marcadas como não-confirmadas e sem ninguém que as pudesse confirmar. **O que fica para aqui:** **(i) Erro engolido** — `resolve-fulfillment.js` falha em silêncio ao gravar a transação ("não falhar a operação por causa da transação"), contra a regra dos 200 de 08/09. Não move dinheiro, por isso não é urgente. **(ii) Tipos duplicados** — `goal-contributions.js` grava `type='spending'` (com um comentário a dizer que `'goal_deposit'` não existe no schema) enquanto `add-money.js` grava `'goal_deposit'`. Dois ficheiros a fazer depósitos em sonhos com tipos diferentes: possível duplicação ou código morto. **(iii) Retenção de logs** — os runtime logs da Vercel duram **1 hora** no plano Hobby. Qualquer incidente não olhado dentro de 60 minutos é impossível de investigar (descoberto com os três 401 das 17:01 UTC de 08/09, encerrados sem conclusão). Decidir se vale a pena exportar logs para outro sítio.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 3.19 | Saldo acumulado do extracto conta movimentos que não aconteceram     | 🔒     | O walk-back do `TransactionHistory.tsx` assume que **toda** a transação moveu `children.balance`. **Duas não movem:** `goal_purchase` (o `resolve-fulfillment.js` mantém o saldo de propósito — o dinheiro saiu quando entrou no sonho) e `goal_interest` (o `applyGoalInterest` chama `adjust_goal_amount`, não `adjust_child_balance`). O walk-back desfaz um passo que nunca foi dado e **inflaciona todas as linhas anteriores** do extracto. **Solução:** marcá-las como registo-não-movimento e saltá-las no walk-back. **Nenhuma existe hoje** — por isso nunca foi visto. **Dispara na primeira contribuição para um sonho.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 3.20 | R$100 possivelmente em falta do Rafael                               | 🔒     | **⚠️ PRIORIDADE: primeira task a seguir ao fecho da 3.14.** A reconciliação de 08/09 (saldo menos soma das transações `completed`) deu **R$1.052,65** no Gabriel e **R$700,00** no Rafael. Os presentes pendentes de cada um são **R$1.050** e **R$800**. O Gabriel fecha com **R$2,65** de resíduo — ruído de arredondamento. O Rafael fica **R$100 abaixo**, e tem exactamente um presente parado de **R$100** (Tio Celso, 30/11/2025). **Hipótese:** ficou de fora do acerto manual de saldos de Março/2026. **NÃO é conclusão** — é a hipótese mais provável, por apurar. Os acertos manuais nunca viraram transações, portanto a reconciliação nunca fecha com rigor total. **MEXE EM DINHEIRO DE UMA CRIANÇA:** diagnóstico → dry-run → aprovação do Tiago → execução → verificação independente. **Sem atalhos.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |

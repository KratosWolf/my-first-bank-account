...
- 🔄 Fase 3 em progresso — 10/17 tasks completas
...
**Status:** 🔄 Em progresso — 10/17 tasks completas (3.1, 3.2, 3.3a, 3.5, 3.6, 3.7, 3.9, 3.10, 3.11, 3.12).
...
| 3.11 | Migrar crons para agendador externo + alerta    | ✅     | Vercel Cron substitui o agendamento do GitHub: `apply-allowance` 08:00 UTC e `apply-interest` 03:00 UTC, **ambos diários**. Juros passam a correr todos os dias de propósito — idempotência por mês + catch-up já garantem crédito único, e se o dia 1 falhar o dia 2 recupera. Watchdog de NÃO-EXECUÇÃO via healthchecks.io (`src/lib/healthcheck.ts`): ping normal em execução limpa, `/fail` em erro, **nenhum ping em dry_run**, nunca lança exceção (timeout 5s, testado contra host morto, body circular e servidor mudo). Os 3 endpoints aceitam GET+POST com `Authorization: Bearer` (o Vercel Cron invoca por GET). Os 3 workflows perderam `schedule:` e mantêm `workflow_dispatch:`; `daily-allowance.yml` passou a `--fail-with-body`. ⚠️ **Desvios ao plano, por o projeto estar em Hobby:** só 2 crons (limite do plano), portanto `keep-alive` ficou como endpoint manual **sem entrada no vercel.json e sem ping** — o banco já recebe tráfego diário dos outros dois. `maxDuration: 60` nas rotas de cron (máximo garantido no Hobby). ⚠️ **Verificação pendente:** a primeira execução real do Vercel Cron é 09/09 03:00 UTC (juros) e 08:00 UTC (mesada). Até lá, nada prova em produção. |

...
export default function FamilyGoals({ familyId, currentChildId }: FamilyGoalsProps) {
  const [familyGoals, setFamilyGoals] = useState<FamilyGoal[]>([]);
  const [contributions, setContributions] = useState<Record<string, FamilyGoalContribution[]>>({});
  ...
  const loadFamilyGoals = async () => {
    try {
      ...
      for (const goal of goals) {
        const goalContributions = await GoalsService.getFamilyGoalContributions(goal.id);
  ...
  const getCategoryInfo = (category: FamilyGoal['category']) => {
    const categories = {
      ...
      house_improvement: { emoji: '🏠', color: 'text-green-700', bgColor: 'bg-green-100' },
      family_activity: { emoji: '🎯', color: 'text-purple-700', bgColor: 'bg-purple-100' },
      emergency_fund: { emoji: '🛡️', color: 'text-red-700', bgColor: 'bg-red-100' },
      education: { emoji: '🎓', color: 'text-indigo-700', bgColor: 'bg-indigo-100' },
    ...
    <div className="bg-white rounded-xl p-6 shadow-lg">
      ...
      {familyGoals.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No Family Goals Yet</h3>
          <p className="text-gray-600">
            Create family goals that everyone can contribute to and achieve together!
          </p>
        </div>
      ) : (
              ...
              >
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${categoryInfo.bgColor} ${categoryInfo.color}`}>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-gray-700">Family Progress</span>
                    <span className="text-sm text-gray-500">
                  ...
                  {!progress.isCompleted && (
                    <p className="text-sm text-gray-500 mt-1">
                      R$ {progress.remaining.toFixed(2)} remaining to reach the family goal
                    </p>
                  )}
                </div>
                ...
                {!goal.is_completed && currentChildId && (
                  <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                    <h4 className="font-semibold text-gray-900 mb-3">Contribute to Family Goal</h4>
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        disabled={contributing[goal.id] || !contributeAmount[goal.id] || parseFloat(contributeAmount[goal.id] || '0') <= 0}
                    ...
                    <div className="mt-3">
                      ...
                      <div className="flex space-x-2">
                        {[25, 50, 100, 200].map(amount => (
                          <button
                            key={amount}
                            onClick={() => setContributeAmount(prev => ({ ...prev, [goal.id]: amount.toString() }))}
                  ...
                  <div>
                    ...
                    <div className="space-y-2 max-h-32 overflow-y-auto">
                      {goalContributions.slice(0, 5).map(contribution => (
                        <div key={contribution.id} className="flex items-center justify-between text-sm bg-gray-50 rounded-lg p-2">
                          <div className="flex items-center space-x-2">
                            <span className="text-lg">{(contribution as any).child?.avatar || '👤'}</span>
                            <span className="font-medium">
                              {(contribution as any).child?.name || 'Family Member'}
                            </span>
                ...
                {goal.is_completed && goal.completed_at && (
                  <div className="mt-4 p-4 bg-green-100 border border-green-200 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-bold text-green-900">Family Goal Achieved!</p>
